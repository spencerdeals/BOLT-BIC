...
let ApifyScraper, UPCItemDB, learningSystem;

try {
  ApifyScraper = require('./apifyScraper');
} catch (error) {
  console.log('⚠️ ApifyScraper not available:', error.message);
  ApifyScraper = null;
}
...
try {
  ...
  console.log('⚠️ UPCItemDB not available:', error.message);
  UPCItemDB = null;
}

try {
  learningSystem = require('./learningSystem');
} catch (error) {
  console.log('⚠️ Learning system not available:', error.message);
  learningSystem = null;
}
...
const upcItemDB = UPCItemDB ? new UPCItemDB(UPCITEMDB_API_KEY) : null;
const USE_UPCITEMDB = !!(UPCITEMDB_API_KEY && UPCItemDB);
...
const apifyScraper = ApifyScraper ? new ApifyScraper(APIFY_API_KEY) : null;
const USE_APIFY = apifyScraper ? apifyScraper.isAvailable() : false;
...
app.get('/scraping-report', async (req, res) => {
  try {
    if (learningSystem) {
      ...
      res.json({ error: 'Learning system not available' });
    }
  } catch (error) {
...
async function scrapeProduct(url) {
  ...
  const knownProduct = learningSystem ? await learningSystem.getKnownProduct(url) : null;
  if (knownProduct) {
  ...
  if (!productData.dimensions) {
    ...
    const aiEstimate = learningSystem ? await learningSystem.getSmartEstimation(category, productName, retailer) : null;
    if (aiEstimate) {
