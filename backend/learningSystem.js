// backend/learningSystem.js - AI Learning System for SDL Import Calculator
const fs = require('fs');
...
try {
  if (fs.existsSync(dbPath)) {
    ...
    console.log('📚 Loaded existing learning database');
  }
} catch (error) {
...
async function initDatabase() {
  try {
    ...
    console.log('✅ AI Learning Database initialized');
    console.log(`📊 Current data: ${db.products.length} products, ${db.category_patterns.length} categories, ${db.retailer_patterns.length} retailers`);
  } catch (error) {
...
class LearningSystem {
  ...
  async getKnownProduct(url) {
    try {
      ...
      if (product) {
        console.log('🧠 Found product in learning system');
        ...
        return {
          ...
          inStock: true,
          scrapingMethod: 'learning_system'
        };
      }
      return null;
    } catch (error) {
  ...
  async saveProduct(product) {
    try {
      ...
      console.log(`💾 Saved product to learning system: ${name?.substring(0, 30)}...`);
  ...
  async cleanup() {
    try {
      ...
      // Remove old products with low confidence
      const originalCount = db.products.length;
      db.products = db.products.filter(p => 
        new Date(p.last_updated) > sixMonthsAgo || p.confidence > 0.8
      );
      
      // Remove old failures
      db.scraping_failures = db.scraping_failures.filter(f => 
        new Date(f.timestamp) > sixMonthsAgo
      );
      
      const cleaned = originalCount - db.products.length;
      if (cleaned > 0) {
        console.log(`🧹 Cleaned up ${cleaned} old products from learning system`);
        saveDatabase();
      }
    ...
    try {
      const exportData = {
        ...
        summary: {
          ...
          totalCategories: db.category_patterns.length,
          totalRetailers: db.retailer_patterns.length,
          totalFailures: db.scraping_failures.length
        },
        data: db
      };
